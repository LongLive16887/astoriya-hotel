// Firebase settings without importing the SDK, so the main bundle stays small:
// the SDK itself is loaded on demand (see publicDb.ts and the admin panel).
...
/** Loads the public data module (Firestore Lite) on first use. */
export const loadPublicDb = () => import('./publicDb')
